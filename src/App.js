...
export default function App() {
    ...
    const currentPageCourses = data.courses.slice(firstCourseIndex, lastCourseIndex)
    const firstRowCouses = currentPageCourses.slice(0, coursesPerRaw)
    const secondRowCouses = currentPageCourses.slice(coursesPerRaw, coursesPerPage)

    const paginate = (pageNumber) => setCurrentPage(pageNumber)

    return (
        <div className="courses-page mt-5">
            <h1 style={{display: 'flex',  justifyContent:'center'}} allign="center" className="primary">Courses</h1>
